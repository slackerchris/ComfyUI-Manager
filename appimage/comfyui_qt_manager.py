...
class ComfyUIManager(QMainWindow):
    ...
    def start_comfyui(self):
        ...
        try:
            ...
            # Use bundled AppImage Python and ComfyUI for complete self-containment
            if self.appdir and os.path.exists(os.path.join(self.appdir, "usr", "bin", "python3")):
                ...
                self.log_display.append(f"✅ Using bundled Python: {python_exe}")
                self.log_display.append(f"✅ Using bundled ComfyUI: {main_py}")
            else:
                # Not running from AppImage - this shouldn't happen for end users
                error_msg = "ERROR: AppImage environment not detected!\n\n"
                error_msg += "This application must be run from the AppImage.\n"
                error_msg += f"APPDIR: {self.appdir}\n"
                error_msg += "Expected: Bundled Python and ComfyUI in AppImage"
            ...
            if self.appdir and os.path.exists(os.path.join(self.appdir, "usr", "bin", "python3")):
                ...
                # Development mode - clean environment
                self.log_display.append("🔧 Using system environment (development mode)...")
    ...
    def stop_comfyui(self):
        ...
        try:
            ...
            if not processes:
                self.log_display.append("⚠️ No ComfyUI processes found to stop")
                # Don't show blocking dialog during restart
    ...
    def restart_comfyui(self):
        ...
        try:
            ...
            # Set flag to prevent blocking dialogs during restart
            self._restarting = True
            
            # Stop current instance
            self.stop_comfyui()
            
            # Clear restart flag and start after delay
            def start_after_stop():
                self._restarting = False
                self.start_comfyui()
            
            QTimer.singleShot(2000, start_after_stop)  # Wait 2 seconds then start
            
        except Exception as e:
            self._restarting = False  # Clear flag on error
